...
private:
    ...
    TCPServer(std::unique_ptr<BaseSocket> device, uint32_t src_ip, uint32_t dest_ip,
              uint16_t src_port, uint16_t dest_port, TCPState status);

    void accept();

public:
    static TCPServer accept(std::unique_ptr<BaseSocket> device, uint32_t src_ip, uint32_t dest_ip,
